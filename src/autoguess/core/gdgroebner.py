...
class ReduceGDtoGroebner:
    ...
    def __init__(self,
        ...
        print('=' * 60)
        print('GRÖBNER BASIS SOLVER — %s' % self.problem_name)
        print('=' * 60)
        print('Variables: %d | Relations: %d' % (self.num_of_vars, self.num_of_relations))
        print('Term ordering: %s | Overlapping: %d' % (self.term_ordering, self.overlapping_number))
        print('CNF→ANF method: %s' % self.cnf_to_anf_conversion)
        print('-' * 60)
        print('POLYNOMIAL RING CONSTRUCTION')
        print('-' * 60)
        print('Generating Boolean Polynomial Ring in %d variables ...' % len(self.variables))
        ...
        print('Ring constructed in %0.02f seconds' % elapsed_time)
    ...
    def generate_horn_clauses(self):
        ...
        elapsed_time = time.time() - starting_time
        print('Horn-SAT problem generated in %0.2f seconds' % elapsed_time)
    ...
    def make_model(self):
        ...
        print('-' * 60)
        print('CNF → ANF CONVERSION (%s)' % self.cnf_to_anf_conversion)
        print('-' * 60)
        ...
        print('ANF representation generated in %0.2f seconds' % elapsed_time)
    ...
    def compute_guess_basis(self):
        ...
        if self.log == 1:
            ...
            print('Main ideal stored at: %s' % main_ideal_path)
        print('-' * 60)
        ...
        elapsed_time = time.time() - starting_time
        print('Gröbner basis computed in %0.2f seconds' % elapsed_time)
        if self.log == 1: 
    ...
    def solve(self):
        ...
        guessed_vars_pretty = []
        for v in self.guessed_vars:
            if hasattr(self, "dummy_mapping") and v in self.dummy_mapping:
                guessed_vars_pretty.append(f"{v} (represents: {' * '.join(self.dummy_mapping[v])})")
            else:
        ...
        print('RESULTS')
        print('=' * 60)
        print(f'Number of guesses:         {len(self.guessed_vars)}')
        print('-' * 60)
        print(f'Guessed variable(s) ({len(self.guessed_vars)}):')
        ...
        guessed_output = []
        for v in self.guessed_vars:
            if self.dummy_mapping and v in self.dummy_mapping:
                guessed_output.append(f"{v} (represents: {' * '.join(self.dummy_mapping[v])})")
            else:
                guessed_output.append(v)
        output_buffer += '\nThe following %d variable(s) are guessed:\n%s\n' % (
            len(self.guessed_vars), ', '.join(guessed_output))
        output_buffer += separator_line
...
def read_relation_file(path, temp_dir, preprocess=1, D=2, log=0, extra_known=None):
    ...
    if log == 0 and preprocess == 1 and algebraic_relations != '':
...
def algebraic_relations_to_connection_relations(algebraic_relations):
    """
    Generate the connection relations derived from the given algebraic relations
    ...
    for monomial in all_monomials:
        if degree_of_monomial(monomial) >= 2:
            ...
            dummy_var = "{0}{1}".format(
                dummy_vars_prefix, "x".join(var_indices))
