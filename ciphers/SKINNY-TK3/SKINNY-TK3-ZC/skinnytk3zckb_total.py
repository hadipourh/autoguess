...
def skinnytk3zckb(R = 23):
    ...
    subkeys_must_be_guessed_for_skinny_zc_64_192 = ['sk_%d_%d' % (16, i) for i in [5]] + \
        ['sk_%d_%d' % (17, i) for i in [0, 6]] + ['sk_%d_%d' % (18, i) for i in [1, 3, 4, 7]] + \
        ['sk_%d_%d' % (19, i) for i in [0, 2, 3, 4, 5, 7]] + \
    ...
    eqs += '\nend'
    relation_file_path = os.path.join(output_dir, 'relationfile_%s_%dr_mg%d_ms%d_total.txt' % (cipher_name, R, recommended_mg, recommended_ms))
