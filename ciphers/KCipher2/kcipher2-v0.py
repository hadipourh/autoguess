...
def k2cipher(T=7):
    ...
    for t in range(T):
        ...
        eqs += 'C1_%d, B_%d => BD_%d\n' % (t, t, t)
        eqs += 'C2_%d, B_%d => BD_%d\n' % (t, 8 + t, 8 + t)
        # FSM Relations:
        ...
        eqs += 'L2_%d, L1_%d\n' % (1 + t, t)
        # Output Relations
        eqs += 'ZL_%d, B_%d, R2_%d, R1_%d, A_%d\n' % (t, t, t, t, 4 + t)
        eqs += 'ZH_%d, B_%d, L2_%d, L1_%d, A_%d\n' % (t, 10 + t, t, t, t)
    eqs += 'known\n' + '\n'.join(['ZL_%d' % i for i in range(T)] + \
                                 ['ZH_%d' % i for i in range(T)])
    eqs += '\nend'
