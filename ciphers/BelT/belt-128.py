...
def belt(R=1):
    cipher_name = 'belt-128'
    # R round
    recommended_mg = 10
    recommended_ms = 10
    rotation = 7
    SK = ['k_%d' % i for i in range(1, 6)] +['k_1', 'k_2', 'k_3', 'k_4']

    
    eqs = '#%s %d Rounds\n' % (cipher_name, R)
    eqs += 'connection relations\n'
    for r in range(0, R):
        eqs += 'w_%d, %s, a_%d\n' % (r, SK[(0 + r*rotation)%8], r)
        eqs += 'a_%d, x_%d, d_%d\n' % (r, r, r)
        eqs += 'y_%d, b_%d, e_%d\n' % (r, r, r)
        eqs += '%s, b_%d, z_%d\n' % (SK[(1 + r*rotation)%8], r, r)
        eqs += 'w_%d, c_%d, y_%d\n' % (r, r, r + 1)
        eqs += 'c_%d, %s, d_%d\n' % (r, SK[(2 + r*rotation)%8], r)
        eqs += 'd_%d, %s, e_%d, f_%d\n' % (r, SK[(3 + r*rotation)%8], r, r)
        eqs += 'd_%d, f_%d, g_%d\n' % (r, r, r)
        eqs += 'f_%d, e_%d, h_%d\n' % (r, r, r)
        eqs += 'h_%d, %s, p_%d\n' % (r, SK[(4 + r*rotation)%8], r)
        eqs += 'p_%d, z_%d, x_%d\n' % (r, r, r + 1)
        eqs += 'y_%d, %s, q_%d\n' % (r + 1, SK[(5 + r*rotation)%8], r)
        eqs += 'q_%d, g_%d, w_%d\n' % (r, r, r + 1)
        eqs += 'h_%d, s_%d, z_%d\n' % (r, r, r + 1)
        eqs += 's_%d, %s, x_%d\n' % (r, SK[(6 + r*rotation)%8], r + 1)
