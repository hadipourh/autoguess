...
def parse_solver_solution(gd):
    ...
    gd.vertices = []
    gd.edges = []
    gd.guessed_vars = [v for v in gd.variables if (gd.solutions[0]['%s_0' % v] == 1) and (v not in gd.known_variables)]
    ...
    while step < gd.max_steps + 1 and not all(determined_target_variables.values()):
        ...
        for rel in gd.symmetric_relations:
            rel_known_vars = [v for v in rel if gd.solutions[step]['%s_%d' % (v, step)] == 1]
